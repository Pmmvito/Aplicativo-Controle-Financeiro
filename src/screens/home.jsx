...
import { View, Text, Button, TextInput, StyleSheet, Modal, TouchableOpacity, ScrollView, Alert } from 'react-native';
...
export default function Home({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      parsedData[monthKey] = { ...parsedData[monthKey], saldo: saldoAtual, transactions: filteredData };
  ...
  const saveData = useCallback(async () => {
    ...
    try {
      ...
      parsedData[monthKey].transactions.push({ type, name, value: parsedValue, date: new Date() });

      const saldoAtual = calculateSaldo(parsedData[monthKey].transactions) + previousMonthSaldo;
  ...
  }, [name, value, type, currentMonth, previousMonthSaldo, calculateSaldo, fetchData, fetchSaldos]);

  const chartData = useMemo(() => ({
    labels: Array.from({ length: 12 }, (_, i) => format(addMonths(startOfYear(new Date()), i), 'MMM')),
    datasets: [
      {
        data: saldos,
      },
  ...
  const totalEntradas = useMemo(() => {
    return data.filter(item => item.type === 'entrada').reduce((sum, item) => sum + item.value, 0);
  }, [data]);

  const totalDespesas = useMemo(() => {
    return data.filter(item => item.type === 'despesa').reduce((sum, item) => sum + item.value, 0);
  }, [data]);
